<div class="js-menu-vendor fixed  z-50 w-screen h-screen top-0 right-0 bg-white hidden">
    @include('layouts.admin.menu-side')
</div>
...
<div class="fixed-header fixed right-0 -top-1 z-40  w-screen ">
    <header class="main-header shadow-md shadow-purple-300 bg-white">
        <div class="flex px-4 py-2 items-center justify-between">
            ...
            <div class="left flex gap-2">
                ...
                @if (App\BadAuth::where('phone', auth()->user()->phone)->exists())
                    <a href="{{ route('vendor_bevendor_vendor_to_user') }}"
                        class="icon text-xs shadow rounded-md px-1.5 mt-2 text-white bg-purple-800 py-2 h-8 flex items-center border-gray-100 border ">
                        اکانت مشتری
                    </a>
                @endif

                <div
                    class="js-menu-vendor-btn icon shadow rounded-md px-1.5 mt-2 text-purple-800 py-2 h-8 flex items-center border-gray-100 border ">
        ...
        <section class="tabs scale-90  pt-3  transform flex justify-between">
            <a class="flex @if (Route::currentRouteName() == 'vendor_index') border-b-2 border-purple-800 @endif  px-3 pb-3  -mb-1  items-center "
                href="{{ route('vendor_index') }}">
            ...
            <a class="flex @if (Route::currentRouteName() == 'vendor_offer_index') border-b-2 border-green-500 @endif  px-3 pb-3  -mb-1  items-center"
                href="{{ route('vendor_offer_index') }}">
                <div class="icon shadow rounded-md px-1.5  py-2 h-8 flex items-center border-gray-100 border "><svg
                ...
                <div class="text text-sm pr-2"> پیشنهادهای شما</div>
            </a>


            <a class="flex @if (Route::currentRouteName() == 'notification_index') border-b-2 border-yellow-500 @endif  px-3 pb-3  -mb-1  items-center"
                href="{{ route('notification_index') }}">
                <div
                    class="icon text-yellow-500 shadow rounded-md px-1.5  py-2 h-8 flex items-center border-gray-100 border ">
                    <ion-icon name="notifications"></ion-icon>
                </div>
                <div class="text text-sm pr-2"> اعلانات</div>
            </a>
