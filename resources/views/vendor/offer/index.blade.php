    ...
    <section class="offers px-4">
        ...
        <div class="list">
            @foreach ($offers as $offer)
                @if (!($offer->created_at->diff(now())->format('%h') > 2 && $offer->status == -11))
                    <form action="{{ route('vendor_order_show', $offer->id) }}">
                        <div data-id="{{ $offer->id }}" class="js-item item bg-white rounded-lg mt-10 shadow-md">
                            <div class="grid h-max grid-cols-9  w-full">
                                ...
                                <div class="col-span-6 ">
                                    <div class="top">
                                        <div class="flex justify-between mx-3 mt-2">
                                            ...
                                            @switch($offer->status)
                                                @case(0)
                                                    <div class="slug  bg-yellow-300 rounded px-2 py-1 text-xs"> بررسی مشتری
                                                    </div>
                                                @break

                                                @case(1)
                                                    <div class="slug  bg-green-300 rounded px-2 py-1 text-xs"> آماده ارسال
                                                    </div>
                                                @break
                                            @endswitch
                                ...
                                <div class="col-span-9">
                                    @if ($offer->status < 0)
                                        <div class="flex pb-4 mt-2 px-4 gap-1 text-sm buttons">
                                            <a disabled aria-disabled="ture" href="#"
                                                class="button cursor-not-allowed bg-yellow-300 rounded px-7 py-1  text-center ">سفارش
                                                کنسل شده</a>
                                            <a href="{{ route('vendor_offer_destroy', [$offer->id]) }}"
                                                class="button cursor-not-allowed bg-gray-800 rounded text-white px-7 py-1  text-center">
                                                حذف
                                            </a>
                                        </div>
                                    @else
                                        <div class="flex pb-4 mt-2 px-4 buttons">
                                            <a href="{{ route('vendor_offer_show', [$offer->id]) }}"
